...
namespace LibraryMSWF.BL
{
    public class UserBL
    {
        ...
        public string UserValidate(string userName, string userEmail, string userPass)
        {
        ...
        public string GetUserName(int userId)
        {
            ...
            return userDAL.TakeUserNameDAL(userId);
        }
        ...
        public string AddUserBL(string userName, string userEmail, string userPass)
        {
            string isUserValid = UserValidate(userName, userEmail, userPass);
            if (isUserValid == "true")
            {
                UserDAL userDAL = new UserDAL();
                string isDone = userDAL.AddUserDAL(userName, userEmail, userPass);
                if (isDone == "false")
                {
                    return "Server error ,";
                }
        ...
        public string UpdateUserBL(int userId, string userName, string userEmail, string userPass)
        {
            string isUserValid = UserValidate(userName, userEmail, userPass);
            if (isUserValid == "true")
            {
                UserDAL userDAL = new UserDAL();
                bool isDone = userDAL.UpdateUserDAL(userId, userName, userEmail, userPass);
                if (isDone != true)
                {
                    return "Server error ,";
                }
